...
public:
	...
	glm::vec3 get_point(float s)
	{
		...
		int pC = pB + 1;
		int pD = pB + 2;
		if (pC > controlPoints.size() - 1) {
			pC = 0;
			pD = 1;
		}
		if (pD > controlPoints.size() - 1) {
			pD = 0;
		}
		float u = s - pB;
...
private:
	...
	void create_track()
	{
		...
		origOrient.origin = get_point(0);
		Orientation prevOrient;
		prevOrient = origOrient;
		for (float i = 0.2; i < controlPoints.size(); i += 0.2) {
			glm::vec3 pos = get_point(i);
			glm::vec3 forward = glm::normalize(pos - prevOrient.origin);
			
			Orientation newOrient;
			newOrient.Front = forward;
			newOrient.Right = glm::normalize(glm::cross(prevOrient.Up, forward));
			newOrient.Up = glm::normalize(glm::cross(forward, newOrient.Right));
			newOrient.origin = pos;
			for (int track = 0; track < 2; track++) {
				makeRailPart(prevOrient, newOrient, glm::vec2(track * 2 - 1, 0));
			}

			float j = i;

			if ((int)(j * 5) % 5 == 1) {
				glm::vec3 track_center = pos;

				glm::vec3 left_1 = track_center + (-1.0f * newOrient.Right + 0.1f * newOrient.Up + 0.1f * newOrient.Front);
				glm::vec3 left_2 = track_center + (-1.0f * newOrient.Right + 0.1f * newOrient.Up - 0.1f * newOrient.Front);
				glm::vec3 left_3 = track_center + (-1.0f * newOrient.Right - 0.1f * newOrient.Up + 0.1f * newOrient.Front);
				glm::vec3 left_4 = track_center + (-1.0f * newOrient.Right - 0.1f * newOrient.Up - 0.1f * newOrient.Front);

				glm::vec3 right_1 = track_center + (1.0f * newOrient.Right + 0.1f * newOrient.Up + 0.1f * newOrient.Front);
				glm::vec3 right_2 = track_center + (1.0f * newOrient.Right + 0.1f * newOrient.Up - 0.1f * newOrient.Front);
				glm::vec3 right_3 = track_center + (1.0f * newOrient.Right - 0.1f * newOrient.Up + 0.1f * newOrient.Front);
				glm::vec3 right_4 = track_center + (1.0f * newOrient.Right - 0.1f * newOrient.Up - 0.1f * newOrient.Front);

				make_triangle(left_1, left_2, right_2, glm::vec2(0.0f, 0.0f), glm::vec2(1.0f, 0.0f), glm::vec2(1.0f, 1.0f), false);
				make_triangle(left_1, right_1, right_2, glm::vec2(0.0f, 0.0f), glm::vec2(0.0f, 1.0f), glm::vec2(1.0f, 1.0f), true);

				make_triangle(left_2, left_4, right_4, glm::vec2(0.0f, 0.0f), glm::vec2(1.0f, 0.0f), glm::vec2(1.0f, 1.0f), false);
				make_triangle(left_2, right_2, right_4, glm::vec2(0.0f, 0.0f), glm::vec2(0.0f, 1.0f), glm::vec2(1.0f, 1.0f), true);

				make_triangle(left_3, left_1, right_1, glm::vec2(0.0f, 0.0f), glm::vec2(1.0f, 0.0f), glm::vec2(1.0f, 1.0f), false);
				make_triangle(left_3, right_3, right_1, glm::vec2(0.0f, 0.0f), glm::vec2(0.0f, 1.0f), glm::vec2(1.0f, 1.0f), true);

				make_triangle(left_4, left_3, right_3, glm::vec2(0.0f, 0.0f), glm::vec2(1.0f, 0.0f), glm::vec2(1.0f, 1.0f), false);
				make_triangle(left_4, right_4, right_3, glm::vec2(0.0f, 0.0f), glm::vec2(0.0f, 1.0f), glm::vec2(1.0f, 1.0f), true);
			}

			prevOrient = newOrient;
		}
		for (int track = 0; track < 2; track++) {
			makeRailPart(prevOrient, origOrient, glm::vec2(track * 2 - 1, 0));
		}      
		...
		//prevOrient = origOrient;
		//for (float i = 0.5; i < controlPoints.size() - 1; i += 1) {
		//	glm::vec3 pos = get_point(i);
		//	glm::vec3 forward = glm::normalize(pos - prevOrient.origin);
	...
	{
		glm::vec3 prev_center = ori_prev.origin + ori_prev.Right * offset.x * 1.0f;
		glm::vec3 prev_1 = prev_center + 0.25f * (-1.0f * ori_prev.Right + ori_prev.Up);
		glm::vec3 prev_2 = prev_center + 0.25f * (ori_prev.Right + ori_prev.Up);
		glm::vec3 prev_3 = prev_center + 0.25f * (-1.0f * ori_prev.Right + -1.0f * ori_prev.Up);
		glm::vec3 prev_4 = prev_center + 0.25f * (ori_prev.Right + -1.0f * ori_prev.Up);

		glm::vec3 curr_center = ori_cur.origin + ori_cur.Right * offset.x * 1.0f;
		glm::vec3 curr_1 = curr_center + 0.25f * (-1.0f * ori_cur.Right + ori_cur.Up);
		glm::vec3 curr_2 = curr_center + 0.25f * (ori_cur.Right + ori_cur.Up);
		glm::vec3 curr_3 = curr_center + 0.25f * (-1.0f * ori_cur.Right + -1.0f * ori_cur.Up);
		glm::vec3 curr_4 = curr_center + 0.25f * (ori_cur.Right + -1.0f * ori_cur.Up);
