...
public:
	...
	float initVelo = sqrt(2 * 9.8f * 6.5f);
	...
	{
		glm::vec3 prevPos = track.get_point(s);
		float velocity = sqrt(2 * (0.5 * pow(initVelo,2) - 9.8 * prevPos.y));
		s += deltaTime * velocity;
		if (s > track.controlPoints.size()) {
			s -= track.controlPoints.size();
		}
		glm::vec3 currPos = track.get_point(s);
		updateCameraVectors();
		glm::vec3 newFront = glm::normalize(currPos - prevPos);
		glm::vec3 newRight = glm::normalize(glm::cross(WorldUp, newFront));
		glm::vec3 newUp = glm::normalize(glm::cross(newFront, newRight));
		Front = newFront;
		Right = newRight;
		Up = newUp;
		Position = currPos + glm::vec3(0, 2, 0);
